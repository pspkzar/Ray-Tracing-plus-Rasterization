...
DlgPass::DlgPass()
                : wxDialog(DlgPass::parent, -1, wxT("Slanger - Pass"),wxDefaultPosition,wxDefaultSize,wxRESIZE_BORDER|wxDEFAULT_DIALOG_STYLE),
				m_Name("Pass Dialog")
{
...
void 
DlgPass::eventReceived(const std::string &sender, const std::string &eventType, curitiba::event_::IEventData *evt) {
...
void DlgPass::updateProperties(Pass *p) {
	...
	// VIEWPORT
	curitiba::render::Viewport *v = p->getViewport();
	...
	std::map<std::string, curitiba::material::MaterialID> mm = p->getMaterialMap();

	wxPGProperty *pgMM;
	pgMM = m_pg->Append(new wxPGProperty(wxT("Material Maps"), wxPG_LABEL));

	std::map<std::string, curitiba::material::MaterialID>::iterator iterMM = mm.begin();
	MaterialID mid;
...
DlgPass::OnProcessPGChange( wxPropertyGridEvent& e)
{
	...
	if (name == wxT("Camera")) {
		...
		if (index == 0)
			...
			p->setViewport(CURITIBA->getViewport(std::string(value.mb_str())));
		}
	}
