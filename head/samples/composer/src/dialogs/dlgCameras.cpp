...
DlgCameras::DlgCameras()
	: wxDialog(DlgCameras::parent, -1, wxT("Slanger - Cameras"),wxDefaultPosition,wxDefaultSize,wxRESIZE_BORDER|wxDEFAULT_DIALOG_STYLE)
                
{
	...
	this->SetTitle(wxT("Slanger - Cameras"));
...
void DlgCameras::updateViewportLabels() {
	...
	std::vector<std::string> *viewports = CURITIBA->getViewportNames();
...
{
	wxPGProperty* topId,*pid2;
	curitiba::scene::Camera *cam;		
	cam = RENDERMANAGER->getCamera(m_active);

	std::string name = Camera::Mat4String[m];
...
void DlgCameras::update() {
	...
	curitiba::scene::Camera *cam;		
	cam = RENDERMANAGER->getCamera(m_active);
	unsigned int proj = cam->getProjectionType();
	...
	vec4 v = cam->getPropfv(Camera::POSITION);
	pg->SetPropertyValue(wxT("Position.X"),v.x);
	pg->SetPropertyValue(wxT("Position.Y"),v.y);
	pg->SetPropertyValue(wxT("Position.Z"),v.z);

	pg->SetPropertyValue(wxT("Direction.Elevation Angle"), curitiba::math::RadToDeg(cam->getElevationAngle()));
	pg->SetPropertyValue(wxT("Direction.ZX Angle"), curitiba::math::RadToDeg(cam->getZXAngle()));

	v = cam->getPropfv(Camera::VIEW_VEC);
	...
	if (curitiba::scene::Camera::PERSPECTIVE == proj) {
...
{
	std::string name = Camera::Mat4String[m];
	const curitiba::math::mat4 mat = cam->getPropm4(m);
	char s[64];
...
void DlgCameras::OnPropsChange( wxPropertyGridEvent& e) {
	...
	curitiba::scene::Camera *cam = RENDERMANAGER->getCamera(m_active);
...
//	pg->ClearSelection();
	...
	if (topProp == "Position") {
		curitiba::math::vec4 v1 = cam->getPropfv(Camera::POSITION);
		if      (prop == "X") {
		...
		if (prop == "Elevation Angle") {
			...
			float rad = curitiba::math::DegToRad(e.GetPropertyValue().GetDouble());
			cam->setElevationAngle(rad);
		}
		...
		vec4 vv = cam->getPropfv(Camera::VIEW_VEC);
	...
	//else if (topProp == "Up") {
	//	curitiba::math::vec3 v1 = cam->getUpVector();
	//	if      (prop == "X") {
	//		v2.set(e.GetPropertyValueAsDouble(),v1.y,v1.z);
	//		cam->setUpVector(v2);
	//	}
	...
	//}	
		...
		if (op == DLG_MI_PERSPECTIVE) {	
			pg->DisableProperty(wxT("Orthogonal Proj:"));
			pg->EnableProperty(wxT("Perspective Proj:"));
			cam->setProjectionType(curitiba::scene::Camera::PERSPECTIVE);
		}
		else {
			pg->DisableProperty(wxT("Perspective Proj:"));
			pg->EnableProperty(wxT("Orthogonal Proj:"));
			cam->setProjectionType(curitiba::scene::Camera::ORTHO);
		}
	}
	else if (prop == "Near") {
...
void DlgCameras::OnActivate(wxCommandEvent& event) {
	...
	CURITIBA->setActiveCameraName(m_active);
}
