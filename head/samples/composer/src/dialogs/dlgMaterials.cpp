...
DlgMaterials::DlgMaterials()
                : wxDialog(DlgMaterials::parent, -1, wxT("Slanger - Materials"),wxDefaultPosition,wxDefaultSize,wxRESIZE_BORDER|wxDEFAULT_DIALOG_STYLE),
...
void DlgMaterials::toolbarLibMatOpen(wxCommandEvent& WXUNUSED(event) ) {
	...
	curitiba::loader::ProjectLoader::loadMatLib(fullName);
...
/* ----------------------------------------------------------------
				...
				EVENTS FROM OTHER DIALOGS AND CURITIBA

-----------------------------------------------------------------*/

void 
DlgMaterials::eventReceived(const std::string &sender, const std::string &eventType, curitiba::event_::IEventData *evt) {
...
void DlgMaterials::setupTexturesPanel(wxSizer *siz, wxWindow *parent) {
	...
	const long repeatInd[] = {TextureSampler::TEXTURE_WRAP_REPEAT,TextureSampler::TEXTURE_WRAP_CLAMP_TO_EDGE, 
					TextureSampler::TEXTURE_WRAP_CLAMP_TO_BORDER, TextureSampler::TEXTURE_WRAP_MIRRORED_REPEAT}; 

	const wxChar* filtersMag[] =  {wxT("NEAREST"),wxT("LINEAR"),NULL};
	const long filtersIndMag[] =  {TextureSampler::TEXTURE_MAG_NEAREST, TextureSampler::TEXTURE_MAG_LINEAR};
	...
	const long filtersIndMin[] =  {TextureSampler::TEXTURE_MIN_NEAREST, TextureSampler::TEXTURE_MIN_LINEAR, 
					TextureSampler::TEXTURE_MIN_NEAREST_MIPMAP_NEAREST,
					TextureSampler::TEXTURE_MIN_NEAREST_MIPMAP_LINEAR,
					TextureSampler::TEXTURE_MIN_LINEAR_MIPMAP_NEAREST,
					TextureSampler::TEXTURE_MIN_LINEAR_MIPMAP_LINEAR};

	const wxChar* compareFunc[] =  {wxT("LEQUAL"), wxT("GEQUAL"), wxT("LESS"),
					wxT("GREATER"), wxT("EQUAL"), wxT("NOTEQUAL"),
					wxT("ALWAYS"), wxT("NEVER"),NULL};
	const long compareFuncInd[] =  {TextureSampler::COMPARE_LEQUAL, TextureSampler::COMPARE_GEQUAL, 
					TextureSampler::COMPARE_LESS, TextureSampler::COMPARE_GREATER, TextureSampler::COMPARE_EQUAL, TextureSampler::COMPARE_NOTEQUAL,
					TextureSampler::COMPARE_ALWAYS, TextureSampler::COMPARE_NEVER};

	const wxChar* compareMode[] =  {wxT("COMPARE_NONE"), wxT("COMPARE_REF_TO_TEXTURE"),NULL};
	const long compareModeInd[] =  {TextureSampler::TEXTURE_COMPARE_NONE, TextureSampler::TEXTURE_COMPARE_REF_TO_TEXTURE};
	...
	const long texTypeInd[] = { Texture::TEXTURE_1D, Texture::TEXTURE_2D, Texture::TEXTURE_3D, Texture::TEXTURE_CUBE_MAP};
	...
	pgTextureProps->Append( new wxEnumProperty(wxT("Texture Type"),wxPG_LABEL,texType,texTypeInd,Texture::TEXTURE_2D));
	...
	pgTextureProps->Append( new wxEnumProperty(wxT("TEXTURE_WRAP_S"),wxPG_LABEL,repeat,repeatInd,0));
	pgTextureProps->Append( new wxEnumProperty(wxT("TEXTURE_WRAP_T"),wxPG_LABEL,repeat,repeatInd,0));
	pgTextureProps->Append( new wxEnumProperty(wxT("TEXTURE_WRAP_R"),wxPG_LABEL,repeat,repeatInd,0));

	pgTextureProps->Append( new wxEnumProperty(wxT("TEXTURE_MAG_FILTER"),wxPG_LABEL,filtersMag,filtersIndMag,0));
	pgTextureProps->Append( new wxEnumProperty(wxT("TEXTURE_MIN_FILTER"),wxPG_LABEL,filtersMin,filtersIndMin,0));

	pgTextureProps->Append( new wxEnumProperty(wxT("TEXTURE_COMPARE_MODE"),wxPG_LABEL,compareMode,compareModeInd,0));
	pgTextureProps->Append( new wxEnumProperty(wxT("TEXTURE_COMPARE_FUNC"),wxPG_LABEL,compareFunc,compareFuncInd,0));
...
void DlgMaterials::setTextureUnit(int index){
	...
	if (texture == NULL) {
		pgTextureProps->SelectProperty(wxT("Texture Unit"));
	 	pgTextureProps->DisableProperty(wxT("TEXTURE_WRAP_S"));
	 	pgTextureProps->DisableProperty(wxT("TEXTURE_WRAP_T"));
	 	pgTextureProps->DisableProperty(wxT("TEXTURE_WRAP_R"));
	 	pgTextureProps->DisableProperty(wxT("TEXTURE_MAG_FILTER"));
	 	pgTextureProps->DisableProperty(wxT("TEXTURE_MIN_FILTER"));
		pgTextureProps->DisableProperty(wxT("TEXTURE_COMPARE_MODE"));
		pgTextureProps->DisableProperty(wxT("TEXTURE_COMPARE_FUNC"));
		return;
	}

	pgTextureProps->SetPropertyValue(wxT("Texture Type"),wxString(texture->getStringDimension().c_str()));
	pgTextureProps->EnableProperty(wxT("TEXTURE_MAG_FILTER"));
	pgTextureProps->EnableProperty(wxT("TEXTURE_MIN_FILTER"));
	pgTextureProps->EnableProperty(wxT("TEXTURE_COMPARE_MODE"));
	pgTextureProps->EnableProperty(wxT("TEXTURE_COMPARE_FUNC"));
	
	wxString texDim;
	texDim.Printf(wxT("%d x %d x %d"),texture->getWidth(),
								texture->getHeight(),
								texture->getDepth());
	pgTextureProps->SetPropertyValue(wxT("Dimensions(WxHxD)"),texDim);

	pgTextureProps->SetPropertyValue(wxT("TEXTURE_WRAP_S"),(long)ts->getTexProp(TextureSampler::TEXTURE_WRAP_S));
	pgTextureProps->SetPropertyValue(wxT("TEXTURE_WRAP_T"),(long)ts->getTexProp(TextureSampler::TEXTURE_WRAP_S));
	pgTextureProps->SetPropertyValue(wxT("TEXTURE_WRAP_R"),(long)ts->getTexProp(TextureSampler::TEXTURE_WRAP_S));

	if (texture->getDimension() == Texture::TEXTURE_1D) {

		pgTextureProps->EnableProperty(wxT("TEXTURE_WRAP_S"));
	 	pgTextureProps->DisableProperty(wxT("TEXTURE_WRAP_T"));
	 	pgTextureProps->DisableProperty(wxT("TEXTURE_WRAP_R"));
	}
	else if (texture->getDimension() == Texture::TEXTURE_2D || texture->getDimension() == Texture::TEXTURE_CUBE_MAP)  {
		pgTextureProps->EnableProperty(wxT("TEXTURE_WRAP_S"));
		pgTextureProps->EnableProperty(wxT("TEXTURE_WRAP_T"));
	 	pgTextureProps->DisableProperty(wxT("TEXTURE_WRAP_R"));
	}
	else {
		pgTextureProps->EnableProperty(wxT("TEXTURE_WRAP_S"));
		pgTextureProps->EnableProperty(wxT("TEXTURE_WRAP_T"));
		pgTextureProps->EnableProperty(wxT("TEXTURE_WRAP_R"));
	}

	pgTextureProps->SetPropertyValue(wxT("TEXTURE_MAG_FILTER"),(long)ts->getTexProp(TextureSampler::TEXTURE_MAG_FILTER));
	pgTextureProps->SetPropertyValue(wxT("TEXTURE_MIN_FILTER"),(long)ts->getTexProp(TextureSampler::TEXTURE_MIN_FILTER));

	pgTextureProps->SetPropertyValue(wxT("TEXTURE_COMPARE_MODE"),(long)ts->getTexProp(TextureSampler::TEXTURE_COMPARE_MODE));
	pgTextureProps->SetPropertyValue(wxT("TEXTURE_COMPARE_FUNC"),(long)ts->getTexProp(TextureSampler::TEXTURE_COMPARE_FUNC));
}
...
void DlgMaterials::OnProcessTexturePropsChange( wxPropertyGridEvent& e) {
	...
	else if (name == wxT("TEXTURE_WRAP_S")) {
		
		ts->setTexProp(TextureSampler::TEXTURE_WRAP_S,value);
	}
	else if (name == wxT("TEXTURE_WRAP_T")) {

		ts->setTexProp(TextureSampler::TEXTURE_WRAP_T,value);
	}
	else if (name == wxT("TEXTURE_WRAP_R")) {

		ts->setTexProp(TextureSampler::TEXTURE_WRAP_R,value);
	}
	else if (name == wxT("TEXTURE_MAG_FILTER")) {

		ts->setTexProp(TextureSampler::TEXTURE_MAG_FILTER,value);
	}
	else if (name == wxT("TEXTURE_MIN_FILTER")) {

		ts->setTexProp(TextureSampler::TEXTURE_MIN_FILTER,value);
	}
	else if (name == wxT("TEXTURE_COMPARE_MODE")) {

		ts->setTexProp(TextureSampler::TEXTURE_COMPARE_MODE,value);
	}
	else if (name == wxT("TEXTURE_COMPARE_FUNC")) {

		ts->setTexProp(TextureSampler::TEXTURE_COMPARE_FUNC,value);
	}
...
void DlgMaterials::addUniform(ProgramValue  &u, int showGlobal) {
	...
	const wxChar* vec4LightComp[] = {wxT("POSITION"), wxT("DIRECTION"), wxT("NORMALIZED_DIRECTION"), wxT("DIFFUSE"), wxT("AMBIENT"), wxT("SPECULAR"), NULL};
	const long vec4LightCompInd[] = {Light::POSITION, Light::DIRECTION, Light::NORMALIZED_DIRECTION, Light::DIFFUSE,
...
void DlgMaterials::updateUniforms(Material *m) {
	...
	std::map<std::string, curitiba::material::ProgramValue> progValues, uniformValues;
	...
	//		addUniform((*progValuesIter).second,m_cbShowGlobalU->GetValue());
			addUniform(pv,false);
			if (pv.getValueType() == Enums::SAMPLER) {
