...
DlgTextureLib::DlgTextureLib()
	: wxDialog(DlgTextureLib::parent, -1, wxString(wxT("Slanger - Materials")),
                ...
                
{
	...
	this->SetTitle(wxT("Slanger - Textures"));
}
...
DlgTextureLib::setupTexturesPanel(wxSizer *siz, wxWindow *parent) {
	...
	std::string *strDims = Texture::getStringDimensions();

	int size = Texture::getArraySize(dims);
	...
	dims = Texture::getInternalFormats();
	strDims = Texture::getStringInternalFormats();

	size = Texture::getArraySize(dims);
...
void DlgTextureLib::updateTextures(int index) {
	...
	curitiba::render::Texture *texture;
...
void DlgTextureLib::setTextureProps(int index){
	...
	curitiba::render::Texture *texture = RESOURCEMANAGER->getTexture(index);
	...
	int v = texture->getInternalFormat();
	pgTextureProps->SetPropertyValue(wxT("Internal Format"),v);

	pgTextureProps->SetPropertyValue(wxT("Type"),texture->getDimension());
...
{
	curitiba::render::Texture *texture = RESOURCEMANAGER->getTexture(m_activeTexture);
...
void DlgTextureLib::OnSavePNG( wxCommandEvent& event) 
{
	...
	curitiba::loader::TextureLoader *loader = curitiba::loader::TextureLoader::create();
