...
void
GlCanvas::setEngine (curitiba::Curitiba* engine)
{
	m_pEngine = engine;
}
...
GlCanvas::setCamera ()
{
	m_pCamera = CURITIBA->getActiveCamera();
	if (m_pCamera) {
		vec4 v = m_pCamera->getPropfv(Camera::NORMALIZED_VIEW_VEC);
		m_Beta = asin(v.y);
...
GlCanvas::Render ()
{
	...
	if (t > 1000) {
		...
		vec4 v;
		if (m_pCamera) 
			v = m_pCamera->getPropfv(Camera::VIEW_VEC);
		
		sprintf (fps, "FPS: %d - Triangles: %d",m_CounterFps, RENDERER->getCounter(curitiba::render::IRenderer::TRIANGLE_COUNTER));
...
/*	if (m_Beta > 1.5f) 
		...
		m_Beta = -1.5f;
*/
	curitiba::event_::CameraOrientation c(m_Alpha, m_Beta);
	curitiba::event_::IEventData *e= curitiba::event_::EventFactory::create("Camera Orientation");
	e->setData(&c);
...
GlCanvas::OnLeftDown (wxMouseEvent& event)
{
	...
	if (m_pCamera) {
		...
		m_Beta = m_pCamera->getElevationAngle();
		m_Alpha = m_pCamera->getZXAngle();
...
GlCanvas::OnLeftUp (wxMouseEvent &event) {
	...
	//					m_RiverAnimation = new curitiba::animation::LinearAnimation (riber, vec3 (0.0f, -delta.y, 0.0f));
