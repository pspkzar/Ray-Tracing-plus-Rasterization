...
std::map<std::string, curitiba::material::ProgramValue>& 
...
Material::setUniformValues() {
	...
	curitiba::render::IProgram *shader = RESOURCEMANAGER->getProgram(m_Shader);
...
Material::prepare ()
{
	...
	}
	{
		...
		if ("" != m_Shader && m_useShader) {
			...
			curitiba::render::IProgram *shader;
			{
				PROFILE("GetShader");
...
void 
Material::addProgramValue (std::string name, curitiba::material::ProgramValue progVal)
{
...
curitiba::material::ColorMaterial& 
