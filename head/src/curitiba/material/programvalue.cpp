...
ProgramValue::Validate(std::string type,std::string context,std::string component)
{
	if (type == "CAMERA") {
		...
		int id;
		curitiba::Enums::DataType dt;
		Camera::getPropTypeAndId(component, &dt, &id);
		return (id != -1);
	}
	else if (type == "LIGHT") {
		...
		int id;
		curitiba::Enums::DataType dt;
		Light::getPropTypeAndId(component, &dt, &id);
		return (id != -1);
	}
		...
		int id;
		curitiba::Enums::DataType dt;
		Texture::getPropTypeAndId(component, &dt, &id);
		return (id != -1);
	}
	else if (type == "DATA") {
...
ProgramValue::ProgramValue (std::string name, std::string type,std::string context,std::string valueof, int id, bool inSpecML) : m_Cardinality (0)
{
	...
	if (0 == type.compare ("CAMERA")) {
		...
		int attr;
		curitiba::Enums::DataType dt;
		Camera::getPropTypeAndId(valueof, &dt, &attr);
		m_ValueOf = attr;
		m_ValueType = dt;
		...
		int attr;
		curitiba::Enums::DataType dt;
		Light::getPropTypeAndId(valueof, &dt, &attr);
		m_ValueOf = attr;
		m_ValueType = dt;
		m_Cardinality = Enums::getCardinality(dt);
		...
		int attr;
		curitiba::Enums::DataType dt;
		Texture::getPropTypeAndId(valueof, &dt, &attr);
		m_ValueOf = attr;
		m_ValueType = dt;
		m_Cardinality = Enums::getCardinality(dt);
		...
		m_ValueOf = USERDATA;
		m_ValueType = curitiba::Enums::getType(context);
		m_Cardinality = curitiba::Enums::getCardinality(m_ValueType);
		...
		if (0 == context.compare("COLOR")) {
			...
			if (m_ValueOf == IRenderer::NORMAL ) {
				m_ValueType = curitiba::Enums::MAT3;
				m_Cardinality = Enums::getCardinality(Enums::MAT3);
			}
			else {
				m_ValueType = curitiba::Enums::MAT4;
...
ProgramValue::getValues (void)
{
   ...
   switch (m_Type) {
	   ...
	   case CAMERA: {
		  ...
		  switch(m_ValueType) {
				case Enums::VEC4:
					return(float *)(&(cam->getPropfv((Camera::Float4Property)m_ValueOf))); 
	   ...
	   case LIGHT: {
			Light *light = RENDERMANAGER->getLight (m_Context);
			   switch(m_ValueType) {
					case Enums::VEC4:
						return(float *)(&(light->getPropfv((Light::Float4Property)m_ValueOf))); 
	   ...
	   case CURRENT: {
		   if ("MATRIX" == m_Context) {
			 ...
			 //  } 
			   ...
			   switch(m_ValueType) {
					case Enums::VEC4:
						return(RENDERER->getLightfvComponent(m_Id, (Light::Float4Property)m_ValueOf)); 
	
					case Enums::FLOAT:
						m_Value[0] = RENDERER->getLightfComponent(m_Id, (Light::FloatProperty)m_ValueOf);
						return m_Value;

					case Enums::ENUM:
						m_IntValue[0] = RENDERER->getLighteComponent(m_Id, (Light::EnumProperty)m_ValueOf);
						return m_IntValue;

					case Enums::BOOL:
						m_IntValue[0] = RENDERER->getLightbComponent(m_Id, (Light::BoolProperty)m_ValueOf);
						return m_IntValue;
...
void 
ProgramValue::setValueType(curitiba::Enums::DataType s) {
