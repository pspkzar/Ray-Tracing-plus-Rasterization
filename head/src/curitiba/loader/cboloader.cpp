...
void 
CBOLoader::loadScene (curitiba::scene::IScene *aScene, std::string &aFilename)
{
	...
	if (!f.is_open()) {
		CURITIBA_THROW ("Cannot open file: %s", aFilename.c_str());
		return;
	}
...
CBOLoader::_writeOctreeByMatSceneObject(SceneObject *so, std::fstream &f) {
	...
	/* Material groups */
	std::vector<curitiba::material::IMaterialGroup*>& materialGroups = aRenderablePtr->getMaterialGroups();
...
CBOLoader::_writeOctreeByMatNode(OctreeByMatNode *n, std::fstream &f) {
	...
	std::map<std::string, curitiba::scene::SceneObject *>::iterator iter;
...
void 
CBOLoader::writeScene (curitiba::scene::IScene *aScene, std::string &aFilename)
{
	...
	for ( ; objIter != sceneObjects.end(); objIter++) {
		...
		// Material groups 
		std::vector<curitiba::material::IMaterialGroup*>& materialGroups = aRenderable.getMaterialGroups();
		std::vector<curitiba::material::IMaterialGroup*>::iterator mgIter;
...
CBOLoader::_writeMaterial(std::string matName, std::string path, std::fstream &f) 
{
	...
	vt = IState::BOOL;
	int numProps = IState::COUNT_STATESETTING;
	f.write (reinterpret_cast<char *> (&numProps), sizeof(int));
	for (int i = 0; i < numProps; i++) {
		bvalue = s->getSetting((IState::StateSetting)i);
		f.write (reinterpret_cast<char *> (&bvalue), sizeof(bool));
	}

	vt = IState::ENUM;
	numProps = IState::COUNT_STATEPROPENUM;
	f.write (reinterpret_cast<char *> (&numProps), sizeof(int));
	for (int i = 0; i < numProps; i++) {
		ivalue = s->getPrope((IState::StatePropEnum)i);
		f.write (reinterpret_cast<char *> (&ivalue), sizeof(int));
	}

	vt = IState::INT;
	numProps = IState::COUNT_STATEPROPINT;
	f.write (reinterpret_cast<char *> (&numProps), sizeof(int));
	for (int i = 0; i < numProps; i++) {
		ivalue = s->getPropi((IState::StatePropInt)i);
		f.write (reinterpret_cast<char *> (&ivalue), sizeof(int));
	}

	vt = IState::FLOAT;
	numProps = IState::COUNT_STATEPROPFLOAT;
	f.write (reinterpret_cast<char *> (&numProps), sizeof(int));
	for (int i = 0; i < numProps; i++) {
		fvalue = s->getPropf((IState::StatePropFloat)i);
		f.write (reinterpret_cast<char *> (&fvalue), sizeof(float));
	}

	vt = IState::FLOAT4;
	numProps = IState::COUNT_STATEPROPFLOAT4;
	f.write (reinterpret_cast<char *> (&numProps), sizeof(int));
	for (int i = 0; i < numProps; i++) {
		v = s->getProp4f((IState::StatePropFloat4)i);
		fvalues[0] = v.x; fvalues[1] = v.y; fvalues[2] = v.z; fvalues[3] = v.w;
		f.write (reinterpret_cast<char *> (fvalues), sizeof(float)*4);
	}

	vt = IState::BOOL4;
	numProps = IState::COUNT_STATEPROPBOOL4;
	f.write (reinterpret_cast<char *> (&numProps), sizeof(int));
	for (int i = 0; i < numProps; i++) {
		bvalues = s->getProp4b((IState::StatePropBool4)i);
...
CBOLoader::_readMaterial(std::string path, std::fstream &f)
{
	...
	for (int i = 0 ;  i < nProps; i++) {
		...
		s->enable((IState::StateSetting)i,bvalue);
	}
	...
	for (int i = 0 ;  i < nProps; i++) {
		...
		s->setProp((IState::StatePropEnum)i,ivalue);
	}
	...
	for (int i = 0 ;  i < nProps; i++) {
		f.read(reinterpret_cast<char *> (&ivalue), sizeof (int));
		s->setProp((IState::StatePropInt)i,ivalue);
	}
	...
	for (int i = 0 ;  i < nProps; i++) {
		...
		s->setProp((IState::StatePropFloat)i,fvalue);
	}
	...
	for (int i = 0 ;  i < nProps; i++) {
		...
		s->setProp((IState::StatePropFloat4)i,fvalues[0],fvalues[1],fvalues[2],fvalues[3]);
	}
	...
	for (int i = 0 ;  i < nProps; i++) {
		...
		s->setProp((IState::StatePropBool4)i,bvalues[0],bvalues[1],bvalues[2],bvalues[3]);
	}
