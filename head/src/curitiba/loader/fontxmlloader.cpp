...
FontXMLLoader::loadFont (Font *aFont, std::string &aFilename)
{
	...
	if (!loadOK) {
		CURITIBA_THROW("Parsing Error -%s- Line(%d) Column(%d) in file: %s", doc.ErrorDesc(), doc.ErrorRow(), doc.ErrorCol(),aFilename.c_str());
	}		
	...
	if (0 == pElem)
		CURITIBA_THROW("Not a XML file: %s", doc.ErrorDesc(), doc.ErrorRow(), doc.ErrorCol(),aFilename.c_str());
	...
	if (numChars == 0)
		CURITIBA_THROW("Zero chars in file: %s", doc.ErrorDesc(), doc.ErrorRow(), doc.ErrorCol(),aFilename.c_str());
