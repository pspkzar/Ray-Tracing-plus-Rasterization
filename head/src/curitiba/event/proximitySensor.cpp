...
void ProximitySensor::eventReceived(const std::string &sender, const std::string &eventType, curitiba::event_::IEventData *evt){
	...
	if ("ACTIVATE" == eventType)
		...
		if(b && !previous) {
			time_t t;
			t=time(NULL);
			curitiba::event_::IEventData *e= curitiba::event_::EventFactory::create("Float");
			e->setData(&t);
...
void 
ProximitySensor::setVec3(unsigned int prop, curitiba::math::vec3 &value)
{
