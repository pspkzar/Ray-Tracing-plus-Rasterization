		...
		for (; sceneObjectsIter != sceneObjects.end(); sceneObjectsIter++) {
			...
			for ( ; matGroupsIter != matGroups.end(); matGroupsIter++) {
				...
				if ((*matGroupsIter)->getIndexData().getIndexSize()) {
					...
					CuritibaBulletMotionState *motionState = new CuritibaBulletMotionState ((*sceneObjectsIter));
...
void 
BulletWorld::setScene (curitiba::scene::IScene *aScene) 
{
	m_pScene = aScene;
}

void 
BulletWorld::_add (float mass, curitiba::scene::SceneObject *aObject, std::string name, curitiba::math::vec3 aVec)
{
	//btCollisionShape *aShape = new btBoxShape (btVector3 (aVec.x, aVec.y, aVec.z));
	btCollisionShape *aShape = new btSphereShape(aVec.z);
	CuritibaBulletMotionState *motionState = new CuritibaBulletMotionState (aObject);
...
void 
BulletWorld::setVelocity (std::string name, curitiba::math::vec3 vel)
{
