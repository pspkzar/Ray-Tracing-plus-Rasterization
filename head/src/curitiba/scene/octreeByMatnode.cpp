...
void 
OctreeByMatNode::updateNodeTransform(curitiba::math::ITransform *t)
{
	std::map<std::string, curitiba::scene::SceneObject *>::iterator iter;
	for (iter = m_pLocalMeshes.begin(); iter != m_pLocalMeshes.end(); ++iter) 
...
void 
OctreeByMatNode::setRenderable (curitiba::render::IRenderable *aRenderable)
{
...
OctreeByMatNode::_unifyLocalMeshes() {
	...
	for (int index = TOPFRONTLEFT; index <= BOTTOMBACKRIGHT; index++) {
		...
		aux = m_pChilds[index];
		if (aux) {
			std::map<std::string, curitiba::scene::SceneObject *>::iterator iter,iter2;
...
OctreeByMatNode::_split() {
	...
	int offSet;
	std::map<std::string, curitiba::scene::SceneObject *>::iterator matIter;
	SceneObject *s;
	std::string name;
	IMaterialGroup *pMaterialGroup;
