...
RenderManager::getPipelineNames(){
	...
	for( std::map<std::string, curitiba::render::Pipeline*>::iterator iter = m_Pipelines.begin(); iter != m_Pipelines.end(); ++iter ) {
...
{
	std::map<std::string, curitiba::scene::IScene*>::iterator sceneIter;
...
RenderManager::getSceneObject(int id) {
	...
	std::vector<curitiba::scene::SceneObject*>::iterator iter;
...
int
RenderManager::pick (int x, int y, std::vector<curitiba::scene::SceneObject*> &objects, curitiba::scene::Camera &aCamera)
{
	...
	//std::vector<curitiba::scene::ISceneObject*>::iterator objIter;
...
void 
RenderManager::setViewport(curitiba::render::Viewport* vp) {

	m_pRenderer->setViewport(vp);
}

void
RenderManager::setRenderMode (curitiba::render::IRenderer::TRenderMode mode) {
...
RenderManager::getCameraNames(){
	...
	for( std::map<std::string, curitiba::scene::Camera*>::iterator iter = m_Cameras.begin(); iter != m_Cameras.end(); ++iter ) {
      names->push_back((*iter).first); 
    }
	return names;
}
...
RenderManager::getLightNames(){
	...
	for( std::map<std::string, curitiba::scene::Light*>::iterator iter = m_Lights.begin(); iter != m_Lights.end(); ++iter ) {
      names->push_back((*iter).first); 
    }
	return names;
}
...
RenderManager::getLight (const std::string &lightName)
{
	if (false == hasLight (lightName)) {
		m_Lights[lightName] = curitiba::scene::LightFactory::create(lightName,"Light");
	}
...
{
	if (false == hasLight (lightName)) {
		m_Lights[lightName] = curitiba::scene::LightFactory::create(lightName,lightClass);
	}
	return m_Lights[lightName];
}
...
RenderManager::getSceneNames()
{
	...
	std::map<std::string, curitiba::scene::IScene*>::iterator iter = m_Scenes.begin();
...
{
	std::map<std::string, curitiba::scene::IScene*>::iterator iter;
