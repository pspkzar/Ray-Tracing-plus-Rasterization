...
using namespace curitiba::materials;
...
LiSPSMAlgorithm::renderScene (IScene *aScene)
{
	...
	for ( ; camerasIter != vSceneCameras.end(); camerasIter++) {
		...
		m_pRenderer->setMatrix (curitiba::render::IRenderer::PROJECTION);
		m_pRenderer->loadIdentity();

		m_pRenderer->setMatrix (curitiba::render::IRenderer::MODELVIEW);
		m_pRenderer->loadIdentity();	
...
LiSPSMAlgorithm::calculateShadow (IScene *aScene, Camera &aCamera)
{
	...
	curitiba::render::Viewport& lightViewport = lightCam->getViewport();
...
LiSPSMAlgorithm::debug (Camera &quadCam)
{
	...
	curitiba::render::Viewport& quadViewport = quadCam.getViewport();
