	...
	Pass(passName)
{
	...
	//	OptixRenderer::setProgram(OptixRenderer::RAY_GEN, o_EntryPoint, "optix/common.ptx", "buffer_camera" );
		...
		CURITIBA_THROW("Optix Error: pass %s [%s]", m_Name.c_str(), e.getErrorString().c_str());
	}
	...
	//catch(optix::Exception& e) {
	//
	//	CURITIBA_THROW("Optix Error: Init pass %s [%s]", m_Name.c_str(), e.getErrorString().c_str());
	//}
}
...
void 
PassOptix::addGlobalAttribute(std::string name, curitiba::material::ProgramValue &p) {
...
void 
PassOptix::setRenderTarget (curitiba::render::RenderTarget* rt)
{
	glGetError();
	if (rt == NULL) {
	...
	curitiba::render::Texture* texID;
	
	try {
		for (unsigned int i = 0; i < n; ++i) {

			texID = rt->getTexture(i);
			int format = texID->getFormat();

			glBindBuffer(GL_PIXEL_UNPACK_BUFFER, o_OutputPBO[i]);
			// need to allow different types
			glBufferData(GL_PIXEL_UNPACK_BUFFER, rt->getWidth()*rt->getHeight()*rt->getTexture(i)->getElementSize(), 0, GL_STREAM_READ);
		...
		CURITIBA_THROW("Optix Error: setting render target %s in pass %s [%s]", 
...
PassOptix::prepare (void)
{
	...
	std::map<std::string, curitiba::material::ProgramValue>::iterator iter;
...
PassOptix::doPass (void)
{
	...
	try {
		...
		//SLOG("%s", e.getErrorString().c_str());
		CURITIBA_THROW("Optix Error: Launching Kernel in pass %s [%s]", m_Name.c_str(), e.getErrorString().c_str());
	}
	...
	for (unsigned int i = 0; i < m_RenderTarget->getNumberOfColorTargets(); ++i) {
		glBindBuffer(GL_PIXEL_UNPACK_BUFFER, o_OutputPBO[i]);
		glBindTexture(GL_TEXTURE_2D, m_RenderTarget->getTexture(i)->getId());
		glPixelStorei(GL_UNPACK_ALIGNMENT, 1);
		glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, 
						m_RenderTarget->getWidth(), m_RenderTarget->getHeight(),
						m_RenderTarget->getTexture(i)->getFormat(),
						m_RenderTarget->getTexture(i)->getType(), 0);
	...
	for ( ; iter != o_OutputDataBuffer.end(); ++iter) {
		...
		void *m;
		m = malloc(t->getElementSize() * t->getWidth()*t->getHeight() );
		glGetTexImage(	GL_TEXTURE_2D, 0, t->getFormat(), t->getType(), m);
 
		glBindTexture(GL_TEXTURE_2D, 0);

		//SLOG("%f %f %f %f", m[0], m[1], m[2], m[3]);
		memset(m, 0, t->getElementSize() * t->getWidth()*t->getHeight() );
		glBufferData(GL_PIXEL_UNPACK_BUFFER, t->getWidth()*t->getHeight()*t->getElementSize(), m, GL_STREAM_READ);
...
PassOptix::setupCamera (void)
{
	...
	const vec4 &v1 = aCam->getPropfv(Camera::POSITION);
	o_Context["eye"]->setFloat(v1.x, v1.y, v1.z);
	const vec4 &v2 = aCam->getPropfv(Camera::NORMALIZED_UP_VEC);
	o_Context["V"]->setFloat(v2.x, v2.y, v2.z);
	const vec4 &v3 = aCam->getPropfv(Camera::NORMALIZED_RIGHT_VEC);
	o_Context["U"]->setFloat(v3.x, v3.y, v3.z);
	const vec4 &v4 = aCam->getPropfv(Camera::NORMALIZED_VIEW_VEC);
...
PassOptix::optixInit() {
	...
	for ( ; scenesIter != m_SceneVector.end(); ++scenesIter) {
		...
		std::vector<curitiba::scene::SceneObject*>::iterator objsIter;
	...
	for ( ; iter != o_InputBuffers.end(); ++iter) {
		try {
			unsigned int id = RESOURCEMANAGER->getTexture(iter->second)->getId();
			if (RESOURCEMANAGER->getTexture(iter->second)->getDimension() == GL_TEXTURE_2D) {
			...
			CURITIBA_THROW("Optix Error: Input Buffer preparation in pass %s [%s]", m_Name.c_str(), e.getErrorString().c_str());
		}
	}
	...
	for ( ; iter2 != o_OutputDataBuffer.end() ; ++iter2) {
		...
		// need to allow different types
		glBufferData(GL_PIXEL_UNPACK_BUFFER, texID->getWidth()*texID->getHeight()*texID->getElementSize(), 0, GL_STREAM_READ);
		glBindBuffer(GL_PIXEL_UNPACK_BUFFER, 0);

		optix::Buffer ob = o_Context->createBufferFromGLBO(RT_BUFFER_OUTPUT, pbo);
		ob->setSize(texID->getWidth(), texID->getHeight());
		// same here (types)
...
/*		std::vector<SceneObject *> objs = sc->getAllObjects();
		for (unsigned int i = 0; i < objs.size(); ++i) {
			...
			try {
				...
				CURITIBA_THROW("Optix Error: Adding scene %s to pass %s (creating buffers from VBOs) [%s]",
			...
			for (unsigned int g = 0; g < mg.size(); ++g) {
				...
				if (mg[g]->getNumberOfPrimitives() > 0) {
					try {
						...
						CURITIBA_THROW("Optix Error: Adding scene %s to pass %s (adding material groups) [%s]",
												sc->getName().c_str(), m_Name.c_str(),e.getErrorString().c_str()); 
					}
				}
			}
		...
		try {
			...
			CURITIBA_THROW("Optix Error: Adding scene %s to pass %s (adding instances to geometry group) [%s]",
												sc->getName().c_str(), m_Name.c_str(),e.getErrorString().c_str()); 
		}
...
PassOptix::getOptixFormat(Texture *t) {
	...
	int nComp = t->getNumberOfComponents();
	int type = t->getType();
