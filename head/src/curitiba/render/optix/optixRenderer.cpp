...
//		p_Context->setEntryPointCount(1);
		...
		CURITIBA_THROW("Optix Creating Context Error [%s]", e.getErrorString().c_str());
	}
	return 0;
}
...
OptixRenderer::setProgram(ProgramTypes aType, int rayType, std::string fileName, std::string proc) {
	...
	try {
		...
		CURITIBA_THROW("Optix Creating Program Error (file: %s proc: %s) [%s]", fileName.c_str(), proc.c_str(), e.getErrorString().c_str());
	}
	...
	try {
		...
		CURITIBA_THROW("Optix Setting Program Error (file: %s proc: %s) [%s]", fileName.c_str(), proc.c_str(), e.getErrorString().c_str());
	}
}