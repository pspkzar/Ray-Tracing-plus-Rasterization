...
OptixMaterialLib::setMaterialProgram(MaterialPrograms aProgram, int rayType, std::string filename, std::string proc) {
	...
	try {
		...
		CURITIBA_THROW("Optix Creating Material Program Error (%s - %s) [%s]", filename.c_str(), proc.c_str(), e.getErrorString().c_str());
	}
}
...
OptixMaterialLib::isDefined(std::string matName) {
	...
	std::map<std::string, curitiba::render::optixRender::OptixMaterial>::iterator iter;
	std::string aux;
...
void 
OptixMaterialLib::addMaterialAttribute(std::string name, curitiba::material::ProgramValue &p) {
...
void 
OptixMaterialLib::addMaterial(curitiba::material::MaterialID aMat) {
...
//		if (m_MaterialProcLib.count(fullMatName))
		...
		CURITIBA_THROW("Optix Creating Material Error (%s:%s) [%s]", 
...
optix::Material &
OptixMaterialLib::getMaterial(curitiba::material::MaterialID aMat) {
...
void
OptixMaterialLib::applyMaterial(optix::GeometryInstance gi, curitiba::material::MaterialID matID) {

	std::string s = matID.getLibName() + ":" + matID.getMaterialName();
	gi->setMaterial(0, m_MaterialLib[s]);

	curitiba::material::Material *mat = MATERIALLIBMANAGER->getMaterial(matID);
