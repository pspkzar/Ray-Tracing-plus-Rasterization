...
void 
GLRenderTarget::addColorTarget (std::string name, std::string internalFormat, std::string format, std::string type)
{
	if (m_Color >= MAXFBOs) 
		return;
	...
	if (m_Samples > 1)
		...
		m_TexId[m_Color] = RESOURCEMANAGER->createTexture 
			(name, internalFormat,format, type, m_Width, m_Height);
...
void 
GLRenderTarget::addDepthTarget (std::string name, std::string internalFormat, std::string format, std::string type, bool noReadAndWrite)
{
	m_Depth = 1;
...
GLRenderTarget::addStencilTarget (std::string name)
{
	...
	if (m_Samples > 1)
		...
		m_TexId[MAXFBOs+1] = RESOURCEMANAGER->createTexture 
			(name, "STENCIL_INDEX8", "STENCIL", "UNSIGNED_BYTE", m_Width, m_Height);
...
GLRenderTarget::addDepthStencilTarget (std::string name)
{
	...
	if (m_Samples > 1)
		...
		m_TexId[MAXFBOs] = RESOURCEMANAGER->createTexture 
			(name, "DEPTH24_STENCIL8", "DEPTH_STENCIL", "UNSIGNED_INT_24_8", m_Width, m_Height);
...
{	
	if (m_RTCount < RenderTarget::RENDERTARGETS){
	  if (-1 == (int) m_RenderTargets[(int) colorAttachment]) {
		...
		glFramebufferTexture2D  (GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0 + (int)colorAttachment, aTexture->getDimension(), aTexture->getId(), 0);
	}
	else {
		return false;
	}
...
{
	glFramebufferTexture2D (GL_FRAMEBUFFER, type, aTexture->getDimension(), aTexture->getId(), 0);
